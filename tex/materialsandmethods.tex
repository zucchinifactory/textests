...
We plot the channel number $ch$ of the very apparent photo peaks against the theoretic energy value $E$ taken from [TODO Praktikumsanleitung] in figure [TODO Abbildung Energy calibration].
...
To calculate the FWHM $\delta ch$ we effectively use the difference between two channels.
With the energy calibration values $a$ and $b$ we get in total:
\begin{align}
    \label{eq:}
    \begin{split}
        \frac{\delta E}{E} &= \frac{ \delta ch }{ch - b}
    \end{split}
    \\
    \label{eq:}
    \begin{split}
        \Delta \frac{\delta E}{E} &=  \frac{ \Delta b }{ b } \cdot \frac{\delta E}{E}
    \end{split}
\end{align}
%
...
\begin{tabular}{lllll}
\toprule
Samlpe & Type & $E_{\text{exp}}$ [MeV] & $E_{\text{theo}}$ [MeV] & Deviation \\
\midrule
...
\bottomrule
\end{tabular}
\captionof{table}{Comparison of experimental and theoretical energies of relevant processes - b: backscattering, c: Compton scattering, p: photo effect. The deviation is calculated from the exact results.}
...
[TODO hier die zahlenwerte nochmal im text angeben?]
\begin{align}
